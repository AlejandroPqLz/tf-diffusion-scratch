...
class DiffusionModel(tf.keras.Model):
    ...
    model (tf.keras.Model): The model to be trained with the diffusion process.
    ...
    predict_step(data):
            The prediction step for the diffusion model.
    plot_samples(num_samples, poke_type):
            Generate and plot samples from the diffusion model.
    forward_diffusion(x_0, t, timesteps, scheduler, beta_start, beta_end, s):
            Simulate the forward diffusion process.
    beta_scheduler(scheduler, timesteps, beta_start, beta_end, s):
            Generate a schedule for beta values according to the specified type.
    ...
    def predict_step(self, data):
        ...
        time.sleep(0.4)
        for t in tqdm(
            reversed(range(1, timesteps)), desc="Sampling sprite", total=timesteps - 1
        ):
            normalized_t = tf.fill(
    ...
    def beta_scheduler(
        ...
        if scheduler == "linear":
            ...
            t = tf.range(0, timesteps + 1, dtype=tf.float32)
